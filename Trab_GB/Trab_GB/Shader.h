...
public:
	...
	// ------------------------------------------------------------------------
	Shader(const char* vertexPath, const char* fragmentPath, const char* tesselationControlShader = nullptr,
		const char* tesselationEvaluationShader = nullptr, const char* geometryPath = nullptr)
	{
		...
		try
		{
			...
			if (tesselationControlShader != nullptr) {
				tcsShaderFile.open(tesselationControlShader);
				tcsShaderStream << tcsShaderFile.rdbuf();
				tcsShaderFile.close();
				tesselationControlShaderCode = tcsShaderStream.str();
			}

			if (tesselationEvaluationShader != nullptr) {
				tesShaderFile.open(tesselationEvaluationShader);
				tesShaderStream << tesShaderFile.rdbuf();
				tesShaderFile.close();
				tesselationEvaluationShaderCode = tesShaderStream.str();
			}
		...
		if (tesselationControlShader != nullptr)
		{
			const char* tcsShaderCode = tesselationControlShaderCode.c_str();
			tesselationControl = glCreateShader(GL_TESS_CONTROL_SHADER);
			glShaderSource(tesselationControl, 1, &tcsShaderCode, NULL);
			glCompileShader(tesselationControl);
			checkCompileErrors(tesselationControl, "TESSELATION CONTROL");
		}
		// tesselation evaluation shader
		if (tesselationEvaluationShader != nullptr) {
			const char* tesShaderCode = tesselationEvaluationShaderCode.c_str();
			tesselationEvaluation = glCreateShader(GL_TESS_EVALUATION_SHADER);
			glShaderSource(tesselationEvaluation, 1, &tesShaderCode, NULL);
